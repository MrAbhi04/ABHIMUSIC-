...
from ABHIMUSIC import YouTube, app
from ABHIMUSIC.core.call import ABHI
from ABHIMUSIC.misc import SUDOERS, db
from ABHIMUSIC.utils.database import (
    is_active_chat,
    is_music_playing,
    is_muted,
    is_nonadmin_chat,
...
from ABHIMUSIC.utils.decorators.language import languageCB
from ABHIMUSIC.utils.formatters import seconds_to_min
from ABHIMUSIC.utils.inline.play import stream_markup, telegram_markup
from ABHIMUSIC.utils.stream.autoclear import auto_clean
from ABHIMUSIC.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await ABHI.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await ABHI.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            try:
                await ABHI.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await ABHI.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_7"])
            ...
            try:
                await ABHI.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
